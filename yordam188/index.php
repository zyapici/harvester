		...
		function loadPage()
		{
			...
			print("<table class='hovertable'>
			         <tr><th>Customer ID</th>
			             ...
			             <th>JSON Files</th>
			             <th>Get Records</th>
			             <th>Create Mark Files</th>
			             <th>Update Interval (Days)</th>
			             <th>Last Upload Date</th>
			             <th>Upload</th>
			             <th>Status</th>
                                     <th>Extra Files</th>
                                     <th>Last # of rec. ILS</th>
                                     <th>Last # of rec. Loaded</th>
                                     <th># of rec. ILS</th>
                                     <th># of rec Loaded</th>
			         </tr>
			");
			...
			if ($handle) {
			    while (($line = fgets($handle)) !== false) {
			    	$column = explode(";", $line);
                                ...
                                if(status($column[0])=="getting records...")
                                {
                                    $myDirectory = opendir($column[0]."/json/");
									$dirArray=array();
                                    ...
                                    if(($indexCount*50)>=NumberofRecordILS($column[0]))
                                    {
                                        unlink($column[0]."/info/status");
                                        $filestatus=$column[0]."/info/status";
			        ...
			        print("<tr onmouseover=\"this.style.backgroundColor='#ffff66'\" onmouseout=\"this.style.backgroundColor='#d4e3e5'\">
			                 ...
			                 <td>
                                         <a href='details.php?file=json&custid=$column[0]'>
                                                 <img src=images/JSON-logo.gif width='40px' height='40px'/>
                                         </a>
                                         <a class='deletejson' onclick='return command(\"delete.php?file=json&custid=$column[0]\")' href='#'>
                                                 <img src=images/delete.png width='40px' height='40px'/>
                                         </a>
                                         </td>");
			                 ...
			                 print("
			                 <td>
                                         <a class='getrecords' onclick='return command(\"getRecords.php?url=$url&custid=$column[0]\")' href='#'>
                                              <img src=images/download.png width='80px' height='40px'/>
                                         </a>
                                         </td>
			                 <td>
                                         <a class='createmrk' onclick='return command(\"createMarcFiles.php?custid=$column[0]\")' href='#'>
			                 ...
			                 <td>
                                         <a class='uploadmrc' onclick='return command(\"upload.php?username=$column[3]&password=$column[4]&custid=$column[0]&extra=$extra\")' href='#'>
                                             <img src=images/upload.png width='80px' height='40px'/>
                                         </a>
                                         </td>
			                 <td class='status'>".status($column[0])."</td>
                                         <td class='extra'>$column[6]</td>
                                         <td>".lastNumberofRecordILS($column[0])."</td>
                                         <td>".lastNumberofRecordLoaded($column[0])."</td>
                                         <td>".NumberofRecordILS($column[0])."</td>
                                         <td>".NumberofRecordLoaded($column[0])."</td>
			               </tr>
			        ");
			    }
			} else {
...
//////////////////////////END_LOAD_PAGE/////////////////////////
    ...
    var deletejsonarray = document.getElementsByClassName('deletejson');
     ...
     for (i = 0; i < statusarray.length; i++) 
     {
      ...
      console.log(today.getHours());
          if(days > updateinterval)
          {
              ...
              {
                  deletejsonarray[i].click();
              }
              if(stat.indexOf('json files were deleted.')>-1 && today.getHours()>13 && today.getHours()<21) 
              {
