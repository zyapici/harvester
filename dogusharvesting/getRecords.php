...
error_reporting(1); 
 ...
 /////////////////////////////////////////LOOK_AT_TOTAL/////////////////////////////////////////// 
		...
		$urltotal="".$url."/webservis.php?islem=sMarcYordamKKN&token=5c947748411d1f36d85039f2ff56a0ab&start=1&rows=1";
                $ch = curl_init();
                    ...
                    while(($line=fgets($handlecatalog))!==FALSE)
                   {
					   if(substr($line, 5, 1)!=" ")
                        ...
                        if(strlen($line) > 9 || $line=="=007  t\n")
                        {
							...
							if(strpos($line,"=856  40")>-1 && strpos($line,"alan=kunyeDemirbasKN_str")>-1)
                                 continue;
							 
					
                            if(strpos($line,"=903  __")>-1 && strpos($url,"katalog.bartin.edu.tr")>-1)
                                 continue;
                            
                            if(strpos($line,"=856  40")>-1 && strpos($line,"udata:image")>-1)
                                 continue;
							
                            if(strpos($line,"=856 41 ")>-1 || strpos($line,"=856  41")>-1)
							{
							...
							if(strpos($line,"=505  ")>-1)
							{
								...
								$lastewrite=str_replace("\r",'',$lastewrite);
								$lastewrite=str_replace("\n",'',$lastewrite);
								$lastewrite=substr($lastewrite, 0, 40000);
								$lastewrite=$lastewrite.".\r\n";
								//fwrite($fileallmarkload, substr($lastewrite, 0, 9200));
								fwrite($fileallmarkload, $lastewrite);
								continue;
							}
							...
							$lastwrite=str_replace("……",' ',$lastwrite);
							$lastwrite=str_replace("Ã³",'',$lastwrite);
							//$lastwrite=substr($lastwrite, 0, 40000);
							//$lastwrite=$lastwrite.".";
                            //fwrite($fileallmarkload, substr($lastwrite, 0, 9200));
							fwrite($fileallmarkload, $lastwrite);
                        }
